...
class UniversalReEncryption:
    ...
    def encrypt_bitmap(self, bitmap_str):
        ...
        # 使用多进程并行加密
        with Pool() as pool:
            ciphertext_list = pool.map(self.encrypt, map(int, bitmap_str))
    ...
    def reencrypt_bitmap(self, ciphertext_list):
        ...
        # 使用多进程并行重加密
        with Pool() as pool:
            reencrypted_list = pool.map(self.reencrypt, ciphertext_list)
    ...
    def decrypt_bitmap(self, ciphertext_list):
        ...
        """
        decrypted_bits = []
        partial_key1 = self.partial_key1
        partial_key2 = self.partial_key2

        # 第一阶段：部分解密
        with Pool() as pool:
            partial_decrypted = pool.map(partial(self.partial_decrypt, partial_key=partial_key1), ciphertext_list)

        # 第二阶段：完全解密
        with Pool() as pool:
            decrypted_bits = pool.map(partial(self.final_decrypt, partial_key=partial_key2), partial_decrypted)
